...
function checkPrereqs() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    CA_DOCKER_IMAGE_VERSION=$(docker run --rm hyperledger/fabric-ca:1.5 fabric-ca-client version | sed -ne 's/ Version: //p' | head -1)
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    infoln "===========${CONTAINER_CLI_COMPOSE} -f compose/$COMPOSE_FILE_CA -f compose/$CONTAINER_CLI/${CONTAINER_CLI}-$COMPOSE_FILE_CA up -d 2>&1"
...
function networkUp() {
  ...
  infoln "-f compose/${COMPOSE_FILE_BASE} -f compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_BASE}"
  infoln "DOCKER_SOCK=============${DOCKER_SOCK}"
  DOCKER_SOCK="${DOCKER_SOCK}" ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} up -d 2>&1
  infoln "=========${DOCKER_SOCK} ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} up -d 2>&1"
